...
id.addEventListener('blur',function(event){
  var joinButton = document.querySelector('.join');
    ...
    emailCheck= false;
    if(emailLengthCheck(email) == false || validateEmail(email) == false  || email.length == 0){
...
passwordInput.addEventListener('blur',function(event){
  ...
  if (checkPassword(password)){ // true라면? 
    ...
    pwCheck = false;
    passwordText.innerText = '*비밀번호는 8자 이상,20자 이하이며, 대문자,소문자,숫자,특수문자를 각각 최소 1개 포함해야 합니다.';
    return false;
  }
